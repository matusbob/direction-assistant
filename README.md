...
- Saved routes: record a route once, marking named points along the way (entrance, platform, stairs, etc., or a custom name). Navigate it again later with turn-by-turn guidance — distance countdown to the next point ("23m, turn right" ... "now turn right"), a spoken confirmation once you've actually turned, a chime and vibration on arrival, and a warning if you drift off the route (with a "back on route" confirmation once you recover).
